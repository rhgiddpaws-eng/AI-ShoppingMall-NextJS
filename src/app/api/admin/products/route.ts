// =============================================================================
// 관리자 상품 API - GET /api/admin/products, POST 신규 등록
// 쿼리: search, category, includeDrafts → 상품 목록. POST: 상품 기본 정보 생성 (status: DRAFT | PUBLISHED)
// =============================================================================
...
export async function GET(request: Request) {
  ...
  const mapped = list.map((p: ProductRow) => {
    ...
    return {
      ...
      price: p.price,
      stock: p.stock,
      status: row.status === "DRAFT" ? "임시저장" : p.stock <= 0 ? "품절" : "판매중",
    }
  })
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await prismaClient.$transaction(async (tx: PrismaTransactionClient) => {
